...
//初始化
signalList.init=function(ctxRoot,gridWidth,gridHeight){
...
//列表
signalList.initList=function(){
    $('#signalList').datagrid({
        url:'/iov/metadata/signal/list',
        loadMsg:'数据加载中...',
        emptyMsg:'无数据',
        width : signalList.width,
        height : signalList.height,
        nowrap : false,
        striped : true,
        collapsible:false,
        fitColumns:true,
        ...
        onDblClickRow:function(rowIndex, row){
            signalList.showDetail(row.id);
        },
        toolbar:[{
            text:'增加',
            iconCls:'icon-add',
            handler:function(){
                signalList.showDetail();
            }
        },{
            text:'编辑',
            iconCls:'icon-edit',
            handler:function(){
                ...
                if(rec.length==0){
                    ...
                    signalList.showDetail(rec[0].id);
                }
            }
        },{
            text:'删除',
            ...
            handler:function(){
                ...
                $.messager.confirm("提示","删除选中的"+count+"行记录吗?",function(r){
                    if(r){
                        ...
                        $.post("/iov/metadata/signal/remove",{"ids":ids},function(data){
                            var gd=$("#signalList");
...
//表单窗口
signalList.initDetailWin=function(){
    $('#detailWin').window({
        width:600,
        height:450,
        top:20,
...
//保存
signalList.save=function(){
    $('#ff').form('submit',{
        url:'/iov/metadata/signal/save',
        onSubmit: function(){
            var ok="false";
            if($('#ff').form('validate')){
                ok = $.ajax({
                    type: "POST",
                    url: "/iov/metadata/signal/check",
...
//展示表单
signalList.showDetail=function(id){
    $('#ff').form('clear');
    if(id){
        $.post("/iov/metadata/signal/get",{"id":id},function(data){
            $('#ff').form('load',data);
        });
    }
...
//查询
signalList.query=function(){
    $("#signalList").datagrid("clearSelections");
    $("#signalList").datagrid('load',{
...
//重置
signalList.resetQuery=function(){
